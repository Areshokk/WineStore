...
public class ControllerUtils {
    ...
    static StringBuilder getUrlBuilder(List<String> urlArray) {
        ...
        if (urlArray.get(0).contains("біле") || urlArray.get(0).contains("червоне")) {
            stringBuilder.append("?gender=" + urlArray.get(0));
        } else {
            stringBuilder.append("?brands=" + urlArray.get(0).replaceAll("\\s+", "+"));
        }

        for (int i = 1; i < urlArray.size(); i++) {
            if (urlArray.get(i).contains("біле") || urlArray.get(i).contains("червоне")) {
                stringBuilder.append("&gender=" + urlArray.get(i));
            } else {
                stringBuilder.append("&brands=" + urlArray.get(i).replaceAll("\\s+", "+"));
            }
        }

//        if (urlArray.get(0).contains("сухе") || urlArray.get(0).contains("солодке")) {
//            stringBuilder.append("?type=" + urlArray.get(0));
//        } else {
//            stringBuilder.append("?brands=" + urlArray.get(0).replaceAll("\\s+", "+"));
//        }
//
//        for (int i = 1; i < urlArray.size(); i++) {
//            if (urlArray.get(i).contains("сухе") || urlArray.get(i).contains("солодке")) {
//                stringBuilder.append("&type=" + urlArray.get(i));
//            } else {
//                stringBuilder.append("&brands=" + urlArray.get(i).replaceAll("\\s+", "+"));
//            }
//        }
